...
| ID | Úkol | Status | Poznámka |
|---|---|---|---|
| A5 | GAN + Latent Navigator | `DONE` | Degradace vyřešena — `assets-src/gan_generated/` doplněno (80 reálných GAN výstupů; do 2026-08-10 v `public/images/`). `scripts/slice-latent.mjs` z nich staví plný 10×8 grid (viz `[r]-[c].webp`), `latent-navigator.js` COLS/ROWS aktualizováno na 10/8. Custom SVG diagram + plný 2D navigator s reálnými snímky |
